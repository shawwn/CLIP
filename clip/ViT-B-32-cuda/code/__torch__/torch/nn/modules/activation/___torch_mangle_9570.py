class MultiheadAttention(Module):
  ...
  in_proj_bias : Tensor
  training : bool
  out_proj : __torch__.torch.nn.modules.linear.___torch_mangle_9569._LinearWithBias
  def forward(self: __torch__.torch.nn.modules.activation.___torch_mangle_9570.MultiheadAttention,
    argument_1: Tensor,
    ...
    _7 = int(embed_dim)
    head_dim = torch.floor_divide(embed_dim, CONSTANTS.c4)
    _8 = int(head_dim)
    _9 = int(head_dim)
    _10 = int(head_dim)
    output = torch.matmul(argument_1, torch.t(_3))
    _11 = torch.chunk(torch.add_(output, _2, alpha=1), 3, -1)
    q, k, v, = _11
    q0 = torch.mul(q, CONSTANTS.c1)
    attn_mask0 = torch.unsqueeze(attn_mask, 0)
    q1 = torch.contiguous(q0, memory_format=0)
    _12 = [_5, int(torch.mul(bsz, CONSTANTS.c4)), _10]
    q2 = torch.transpose(torch.view(q1, _12), 0, 1)
    _13 = torch.contiguous(k, memory_format=0)
    _14 = [-1, int(torch.mul(bsz, CONSTANTS.c4)), _9]
    k0 = torch.transpose(torch.view(_13, _14), 0, 1)
    _15 = torch.contiguous(v, memory_format=0)
    _16 = [-1, int(torch.mul(bsz, CONSTANTS.c4)), _8]
    v0 = torch.transpose(torch.view(_15, _16), 0, 1)
    attn_output_weights = torch.bmm(q2, torch.transpose(k0, 1, 2))
    input = torch.add_(attn_output_weights, attn_mask0, alpha=1)
    input0 = torch.softmax(input, -1, None)
    attn_output_weights0 = torch.dropout(input0, 0., True)
    attn_output = torch.bmm(attn_output_weights0, v0)
    _17 = torch.contiguous(torch.transpose(attn_output, 0, 1), memory_format=0)
    input1 = torch.view(_17, [_4, _6, _7])
    output0 = torch.matmul(input1, torch.t(_1))
    return torch.add_(output0, _0, alpha=1)
  def forward1(self: __torch__.torch.nn.modules.activation.___torch_mangle_9570.MultiheadAttention,
    argument_1: Tensor,
    attn_mask: Tensor) -> Tensor:
