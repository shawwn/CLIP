class Multimodal(Module):
  ...
  def encode_text(self: __torch__.multimodal.model.multimodal_transformer.___torch_mangle_9591.Multimodal,
    ...
    x = torch.to(_5, torch.device("cuda:0"), 5, False, False, None)
    _6 = torch.to(_4, torch.device("cuda:0"), 5, False, False, None)
    x0 = torch.add(x, _6, alpha=1)
    x1 = torch.permute(x0, [1, 0, 2])
    x2 = torch.permute((_3).forward(x1, ), [1, 0, 2])
    x3 = torch.to((_2).forward(x2, ), torch.device("cuda:0"), 5, False, False, None)
    ...
    _10 = torch.to(_8, dtype=4, layout=0, device=torch.device("cuda:0"), pin_memory=None, non_blocking=False, copy=False, memory_format=None)
    _11 = torch.to(_9, dtype=4, layout=0, device=torch.device("cuda:0"), pin_memory=None, non_blocking=False, copy=False, memory_format=None)
    _12 = annotate(List[Optional[Tensor]], [_10, _11])
    _13 = torch.matmul(torch.index(x3, _12), _1)
    return _13
  def forward(self: __torch__.multimodal.model.multimodal_transformer.___torch_mangle_9591.Multimodal,
    ...
    x = torch.to((_19).forward1(input, ), torch.device("cuda:0"), 5, False, False, None)
    _22 = torch.to(_18, torch.device("cuda:0"), 5, False, False, None)
    x4 = torch.add(x, _22, alpha=1)
    x5 = torch.permute(x4, [1, 0, 2])
    x6 = torch.permute((_17).forward1(x5, ), [1, 0, 2])
    x7 = torch.to((_16).forward1(x6, ), torch.device("cuda:0"), 5, False, False, None)
    ...
    _26 = torch.to(_24, dtype=4, layout=0, device=torch.device("cuda:0"), pin_memory=None, non_blocking=False, copy=False, memory_format=None)
    _27 = torch.to(_25, dtype=4, layout=0, device=torch.device("cuda:0"), pin_memory=None, non_blocking=False, copy=False, memory_format=None)
